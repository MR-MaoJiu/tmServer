...
use app\api\validate\RegistValidate;
...
class Login extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            if ($request->isPost()) {
                $data = $request->post();
                $request_time = $request->post('request_time') ? $request->post('request_time') : time();
                $user_token = $request->header('token');
                //$info = $this->db->where(['phone' => $data['phone'], 'password' => md5($data['password'])])->find();
                $info = $this->db->where(['status' => 1, 'phone' => $data['phone'], 'password' => md5($data['password'])])->find();
                $update['last_login_time'] = time();
                $update['ip'] = $data['ip'];
                $update['login_count'] = $info['login_count'] + 1;
                if (!$info) return json(['code' => 400, 'msg' => '账号或密码错误']);
                if ($user_token) {
                    $token = $this->getToken($info['uid'], $request_time);
                } else {
                    $token = $this->getToken($info['uid'], time());
                }
                $res = $this->db->update($update, ['uid' => $info['uid']]);
                return json(['code' => 1, 'msg' => '登录成功', 'token' => $token], 200);
                return json(['code' => 0, 'msg' => 'error'], 400);
            }

        } catch (Exception $e) {
            return json(['code' => 0, 'msg' => $e->getMessage()], 400);
        }
    ...
    public function regist(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            $data = $request->post();
            if (!$data) {
                return json(['code' => 400, 'msg' => '没有数据']);
            }
            $info = $this->db->where('phone', $data['phone'])->find();
            if ($info) {
                return json(['code' => 400, 'msg' => '该手机号已注册']);
            }
            if ($data['mobile_code'] != redis()->get('mobile_code')) {
                return json(['code' => '400', 'msg' => '验证码错误请重新输入']);
            }
//            if (session('sms_send_exp') <= session('sms_send_time')+10*60){
//                return json(['code'=>400,'msg'=>'验证码已过期']);
//            }
            if (!$this->rvalidate->check($data)) {
                return json(['code' => 400, 'msg' => $this->rvalidate->getError()]);
            }
            $res = $this->db->insertGetId(['phone' => $data['phone'], 'password' => md5($data['password']), 'create_time' => time(), 'nickname' => $nicheng]);
            $info = [];
            $info['uid'] = $res;
            return json(['code' => 1, 'msg' => '注册成功', 'data' => $info]);
        } else {
    ...
    public function getCode(Request $request)
    {
        if ($request->isPost()) {
            ...
            return (new Common)->send_sms($data['phone']);
        } else {
            return json(['code' => 400, 'msg' => '请求不合法']);
        }
    }
     ...
     * @return \think\response\Json
     */
    public function getToken($id, $exptime)
    {
